...
func addShadowView(width: CGFloat = 2.0, height: CGFloat = 2.0, opacidade: Float = 1.0, maskToBounds: Bool = false, radius: CGFloat = 5, color: UIColor = .gray) {
...
extension UIImage {
    ...
    class func animatedImage(withSource source: CGImageSource) -> UIImage? {
    ...
    class func delayForImageAtIndex(_ index: Int, source: CGImageSource) -> TimeInterval {
        ...
        let properties = CGImageSourceCopyPropertiesAtIndex(source, index, nil) as Dictionary?
        if let gifProperties = properties?[kCGImagePropertyGIFDictionary as String as NSObject] as? NSDictionary {

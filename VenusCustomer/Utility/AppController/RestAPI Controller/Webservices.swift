...
extension WebServices {
    ...
    // MARK: - Common PUT API with Image
    static func commonPUTAPIWithImage(endPoint: EndPoint,
                                       parameters:[String : Any],
                                       image : [String: UIImage],
                                       loader: Bool = true,
                                       response: @escaping APIResponse) {
        ...
        let endPoint = endPoint.path
        AppNetworking.PUTWithImage(endPoint: endPoint, parameters: parameters, image: image , loader: loader) { (result) in
            switch result {
            case .success(let json):
                let code = TDApiCode(rawValue: json["flag"].intValue) ?? .undetermined
                switch code {

...
protocol navigateToEndRideFromChat{
    func navigateToEndRide()
}

class VDChatVC: VCBaseVC, UITextViewDelegate {
    ...
    var delegate: navigateToEndRideFromChat?
    var tripID = ""
    var driverID = ""
    var objGetAllMessages : GetAllMessages?
    var name = ""
    var profileImg = ""
    var txtArr = ["Hello?","Where are you?","Reached at pickup?","Waiting at pickup."]
    ...
    override func initialSetup() {
        ...
        VCSocketServices.shared.listenGetAllMessage()
        NotificationCenter.default.addObserver(self, selector: #selector(handleNotificationn(_:)), name: .getAllMessages, object: nil)
    ...
    @objc func rideEndedAction(_ notification: Notification) {
        self.delegate?.navigateToEndRide()
    ...
    @objc func updateSingleMessage(_ notification: Notification) {
        ...
        do {
            ...
            if self.objGetAllMessages == nil {
               // self.objGetAllMessages = GetAllMessages(from: [ThreadData])
            }
    ...
    @objc func handleNotificationn(_ notification: Notification) {
